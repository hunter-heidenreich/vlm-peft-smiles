...
class VLMPredictor:
    ...
    def evaluate_dataset(
        ...
        **sampling_kwargs
        ...
        try:
            # Prepare input data
            image_paths = [temp_paths[mol.filename] for mol in molecules if mol.filename in temp_paths]
            text_prompts = [text_prompt] * len(image_paths)
            ground_truth = [mol.smiles for mol in molecules if mol.filename in temp_paths]
            
            # Adjust molecules list to match available images
            available_molecules = [mol for mol in molecules if mol.filename in temp_paths]
            
            if len(image_paths) != len(molecules):
                logger.warning(f"Only {len(image_paths)}/{len(molecules)} images were successfully extracted")
            ...
            results_df['exact_match'] = results_df['smiles_ground_truth'] == results_df['smiles_predicted']
            ...
            try:
                ...
                results_df['smiles_ground_truth_canonical'] = results_df['smiles_ground_truth'].apply(canonicalize_smiles)
                results_df['smiles_predicted_canonical'] = results_df['smiles_predicted'].apply(canonicalize_smiles)
                results_df['canonical_match'] = (
                    results_df['smiles_ground_truth_canonical'] == results_df['smiles_predicted_canonical']
                )
